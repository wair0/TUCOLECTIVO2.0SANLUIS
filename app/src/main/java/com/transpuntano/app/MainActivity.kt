...
class MainActivity : AppCompatActivity() {
    ...
    private fun loadNearbyList(list: LinearLayout) {
        ...
        if (location == null) {
            ...
            list.removeAllViews()
            list.addView(panel("UBICACIÓN NO DISPONIBLE", "Activá la ubicación e intentá nuevamente."))
            return
        }
    ...
    private fun loadNearbyOnMap(map: CyberMapView, info: TextView) {
        ...
        fun useLocation(location: android.location.Location?) {
            ...
            runOnUiThread {
                ...
                info.text = "MAPA  •  UBICACIÓN ACTUAL" + accuracyText + "  •  BUSCANDO PARADAS"
            }
            executor.execute {
                runCatching { api.getNearby(location.latitude, location.longitude) }
                    .onSuccess { stops ->
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            ...
            manager.getCurrentLocation(provider, null, mainExecutor) { location ->
                ...
                else if (provider != LocationManager.NETWORK_PROVIDER && manager.isProviderEnabled(LocationManager.NETWORK_PROVIDER)) {
                    manager.getCurrentLocation(LocationManager.NETWORK_PROVIDER, null, mainExecutor) { networkLocation -> useLocation(networkLocation) }
    ...
    private fun cyberSyncButton(action: () -> Unit): ImageView {
        val image = ImageView(this).apply {
            val bitmap = assets.open("sincronizar_lineas.webp").use {
                BitmapFactory.decodeStream(it)
            }
            if (bitmap == null) {
                throw IllegalStateException("No se pudo decodificar sincronizar_lineas.webp")
            }
            setImageBitmap(bitmap)
            scaleType = ImageView.ScaleType.FIT_XY
