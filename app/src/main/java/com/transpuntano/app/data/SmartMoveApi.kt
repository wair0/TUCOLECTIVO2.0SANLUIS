...
class SmartMoveApi {
    ...
    fun getLines(): List<TransitLine> {
        ...
        val array = firstArray(root, "JsonLineas", "listaLineas", "lineas", "Linea", "linea", "lineasJson") ?: throw SmartMoveException("RecuperarLineaPorCuandoLlega: no se encontró una lista de líneas. Respuesta: " + root.toString().take(1200))
        return (0 until array.length()).mapNotNull { index ->
            val item = array.optJSONObject(index) ?: return@mapNotNull null
            val codeText = item.optStringAny("codigoLinea", "CodigoLinea", "CodigoLineaParada", "codigoLineaParada", "codLinea", "codLineaParada", "CODIGOLINEA")
            val code = codeText?.trim()?.toDoubleOrNull()?.toInt() ?: return@mapNotNull null
            TransitLine(
                code,
                item.optStringAny("descripcionLinea", "DescripcionLinea", "Descripcion", "descripcion", "Nombre", "nombre", "descripcionLineaParada", "DescripcionLineaParada").orEmpty().ifBlank { "Línea " + code },
    ...
    private fun firstArray(root: JSONObject, vararg names: String): JSONArray? {\n        val wanted = names.map { it.lowercase(Locale.US) }.toSet()\n        var fallbackArray: JSONArray? = null\n\n        fun fromValue(value: Any?): JSONArray? {\n            when (value) {\n                is JSONArray -> {\n                    if (fallbackArray == null) fallbackArray = value\n                    return value\n                }\n                is JSONObject -> {\n                    val keys = value.keys()\n                    while (keys.hasNext()) {\n                        val key = keys.next()\n                        val child = value.opt(key)\n                        if (key.lowercase(Locale.US) in wanted) {\n                            if (child is JSONArray) return child\n                            if (child is String) {\n                                val s = child.trim()\n                                if (s.startsWith("[")) runCatching { return JSONArray(s) }\n                                if (s.startsWith("{")) runCatching { fromValue(JSONObject(s))?.let { return it } }\n                            }\n                        }\n                        fromValue(child)?.let { found ->\n                            if (key.lowercase(Locale.US) in wanted) return found\n                        }\n                    }\n                }\n                is String -> {\n                    val s = value.trim()\n                    if (s.startsWith("[")) runCatching {\n                        val parsed = JSONArray(s)\n                        if (fallbackArray == null) fallbackArray = parsed\n                        return parsed\n                    }\n                    if (s.startsWith("{")) runCatching { return fromValue(JSONObject(s)) }\n                }\n            }\n            return null\n        }\n\n        return fromValue(root) ?: fallbackArray\n    }ge com.transpuntano.app.data
