...
class CyberMapView(context:Context):View(context){
 ...
 private fun download(key:String,x:Int,y:Int,z:Int){synchronized(cache){if(cache.containsKey(key))return};Thread{runCatching{val h=URL("https://tile.openstreetmap.org/$z/$x/$y.png").openConnection() as HttpURLConnection;h.connectTimeout=6000;h.readTimeout=6000;h.setRequestProperty("User-Agent","TU-COLECTIVO-2.0 Android");h.inputStream.use{BitmapFactory.decodeStream(it)}}.getOrNull()?.let{b->synchronized(cache){cache[key]=b};postInvalidate()}}.start()}
